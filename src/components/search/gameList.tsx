...
function GameList({ pages, size, name }: GameListProps) {
    ...
    if (isLoading || isFetching) {
        ...
        content = data.results.map(game => <div key={game.id} className='search__item'>
            <img className='search__img' src={game.background_image} alt={game.name} />
            <h2>{game.name}</h2>
    ...
    return (
        <div className={(name) ? 'search__list' : 'search__list search__list_hidden'}>
            {content}
        </div>
    )
}
